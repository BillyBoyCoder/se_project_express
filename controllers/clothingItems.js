...
const createItem = (req, res) => {
  ...
  ClothingItem.create({ name, weather, imageUrl, owner })
    ...
    .catch((err) => res.status(500).send({ message: "Error creating item", error: err.message }));
};
...
const dislikeItem = (req, res) => {
  ClothingItem.findByIdAndUpdate(
    ...
    .catch((err) => res.status(500).send({ message: "Error disliking item", error: err.message }));
};
